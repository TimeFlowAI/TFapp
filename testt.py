...
Window.size = (430, 932)
Window.clearcolor = (28 / 255, 86 / 255, 170 / 255, 1)
...
class DayScheduleScreen(Screen):
    def __init__(self, day, month, year, **kwargs):
        super(DayScheduleScreen, self).__init__(**kwargs)
        self.day = day
        self.month = month
        self.year = year
        ...
        # Здесь можно добавить виджеты для расписания дня
        schedule_label = Label(
            text=f"Расписание на {day}.{month}.{year}",
            font_size=dp(18),
...
class CalendarWidget(BoxLayout):
    ...
    displayed_date = ObjectProperty(datetime.now().date())
    ...
    def update_current_date(self):
        self.current_date = self.displayed_date.strftime('%B %Y')
    ...
    def update_calendar(self):
        ...
        month_days = calendar.monthrange(self.displayed_date.year, self.displayed_date.month)[1]
        first_day = datetime(self.displayed_date.year, self.displayed_date.month, 1)
        ...
        for day in range(1, month_days + 1):
            btn = Button(
                ...
                size_hint_y=None,
                height=dp(40),
                on_press=lambda instance, d=day: self.open_day_schedule(d)
            )
            grid.add_widget(btn)
    ...
    def prev_month(self, instance):
        self.displayed_date = (
                    datetime(self.displayed_date.year, self.displayed_date.month, 1) - timedelta(days=1)).date()
        self.update_current_date()
        self.update_calendar()

    def next_month(self, instance):
        next_month = self.displayed_date.month + 1
        next_year = self.displayed_date.year
        if next_month > 12:
            next_month = 1
            next_year += 1
        self.displayed_date = datetime(next_year, next_month, 1).date()
        self.update_current_date()
        self.update_calendar()

    def open_day_schedule(self, day):
        screen_name = f"day_{day}_{self.displayed_date.month}_{self.displayed_date.year}"

        # Проверяем, существует ли уже экран для этого дня
        if not self.parent.parent.manager.has_screen(screen_name):
            day_screen = DayScheduleScreen(
                day=day,
                month=self.displayed_date.month,
                year=self.displayed_date.year,
                name=screen_name
            )
            self.parent.parent.manager.add_widget(day_screen)
...
class CalendarScreen(BaseScreen):
    def __init__(self, **kwargs):
        ...
        self.layout.add_widget(self.calendar_widget)
